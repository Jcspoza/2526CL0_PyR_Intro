...
# F.1
def add_dot():
    """Añade un 'dot' aleatoriamente , solo si antes habia un cuadrado"""
    ...
    if TodosRed:
        ...
        puntosW.value = "¡Fin! Has conseguido, puntos: " + str(Puntos)
    else:
...
# 1 - App 
apl = App("Taller Prog y Robotica CMM BML- Juego: 'Destruye los Puntos'",width=600, height=600)

# 2- wg#1 - Añade un widget Text 
instructiones = Text(apl, text="Clica los puntos para destruirlos")

# 3- wg#2 - Añade un widget Waffle
board = Waffle(apl, width=GRID_SIZE, height=GRID_SIZE, dim=80, command=destroy_dot)
